...
class Article
{
  ...
  {
    $sql = 'SELECT a.*, m.nom AS media_nom, m.datatype AS media_datatype, m.alt AS media_alt, m.credit AS media_credit FROM articles a LEFT JOIN medias m ON m.id = a.id_media WHERE a.id = :id;';
  ...
  {
    $sql = 'SELECT e.*, c.nom AS nom_classe, m.nom AS media_nom, m.datatype AS media_datatype, m.alt AS media_alt, m.credit AS media_credit FROM elements e LEFT JOIN classes c ON c.id = e.id_classe LEFT JOIN medias m ON m.id = e.id_media WHERE id_article = :id ORDER BY ordre;';
  ...
  function create()
  {
    ...
    $sql = 'INSERT INTO articles (nom, chapo, auteur, id_page, id_media) VALUES (:nom, :chapo, :auteur, :id_page, :id_media);';
  ...
  {
    // construction de la requête :nom, :prenom sont les valeurs à insérées
    $sql = 'UPDATE articles SET id_page = :id_page, id_media = :id_media, nom = :nom, chapo = :chapo, auteur = :auteur WHERE id = :id';
    // connexion à la base de données
    $pdo = connexion();
