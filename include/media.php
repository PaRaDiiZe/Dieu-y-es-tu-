...
class Media {
  ...
  {
    $sql= 'SELECT e.*, m.nom AS media_nom, m.datatype AS media_datatype, m.alt AS media_alt, m.credit AS media_credit FROM elements e LEFT JOIN medias m ON e.id_media = m.id WHERE e.id = :id;';
  ...
  function upload()
  {
    ...
    $uploadPath = $currentDirectory . $uploadDirectory . $this->datatype . '/' .basename($fileName); 
    ...
    $sql = 'INSERT INTO medias (nom, chemin, datatype, alt, credit) VALUES (:nom, :chemin, :datatype, :alt, :credit);';
