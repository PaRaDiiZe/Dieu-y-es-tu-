...
class Element
{
  ...
  function chargePOST()
  {
    ...
    if (isset($_POST['sous_contenu'])) {
      $this->sousContenu = $_POST['sous_contenu'];
    } else {
      $this->sousContenu = '';
    }
  ...
  function create()
  {
    ...
    $sql = 'INSERT INTO elements (id_article, balise, contenu, sous_contenu, id_media, href, ordre, id_classe) VALUES (:id_article, :balise, :contenu, :sous_contenu, :id_media, :href, :order, :id_classe);';
    ...
    $query->bindValue(':sous_contenu', $this->sousContenu, PDO::PARAM_STR);
  ...
  {
    // construction de la requête :nom, :prenom sont les valeurs à insérées
    $sql = 'UPDATE elements SET balise = :balise, contenu = :contenu, sous_contenu = :sous_contenu, id_media = :id_media, href = :href, id_classe = :id_classe, id_article = :id_article WHERE id = :id';
    // connexion à la base de données
    $pdo = connexion();
